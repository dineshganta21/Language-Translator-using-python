...
#set title,size, color to our translator
root=Tk()
...
#translating function which we taken it from google trans
def translate_now():
    global language
    try:
        text_=text1.get(1.0,END)
        t1=Translator()
        trans_text=t1.translate(text_,src=combo1.get(),dest=combo2.get())		#src='english',dest='telugu')
        trans_text=trans_text.text

        text2.delete(1.0,END)
        text2.insert(END,trans_text)
    except Exception as e:
        messagebox.showerror("translator","try again")
...
combo2=ttk.Combobox(root,value=languageV,font="Roboto 14",state="r")
